...
def test_fallback_on_model_error(mock_image_open, mock_add_boundary, mock_load_models, setup_test_images, mock_image):
...
@patch("detect_logo.add_boundary")
@patch("PIL.Image.open")
def test_fallback_on_no_predictions(mock_image_open, mock_add_boundary, mock_load_models, setup_test_images, mock_image):
    # Mock Image.open to return a test image
    test_img = Image.new('RGB', (100, 100), color='red')
    test_img.convert = MagicMock(return_value=test_img)
    mock_image_open.return_value = test_img
