...
class ConnectionManager:
    ...
    async def connect(self, batch_id: str, websocket: WebSocket):
        ...
        logger.info(f"New WebSocket connection for batch {batch_id}. Total connections: {len(self.active_connections[batch_id])}")
    ...
    def disconnect(self, batch_id: str, websocket: WebSocket):
        ...
        if batch_id in self.active_connections:
            if websocket in self.active_connections[batch_id]:
                ...
                logger.info(f"WebSocket disconnected from batch {batch_id}. Remaining connections: {len(self.active_connections[batch_id])}")
    ...
    async def broadcast(self, batch_id: str, message: Any):
        ...
        if batch_id in self.active_connections:
            ...
            logger.debug(f"Broadcast message to {len(self.active_connections[batch_id])} connections for batch {batch_id}")
